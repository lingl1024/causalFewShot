...
def bert_zeroshot_classifier(classnames, templates, model):
    with torch.no_grad():
        zeroshot_weights = []
        for classname in classnames:
            texts = [template.format(classname) for template in templates]  # format with class
            texts = model.tokenizer(texts, padding='max_length', truncation=True, max_length=35, 
                              return_tensors="pt").to(torch.device('cuda')) 
            class_embeddings = model.text_encoder(texts.input_ids, attention_mask = texts.attention_mask, return_dict = True, mode = 'text')  # embed with text encoder
            class_embeddings = class_embeddings.last_hidden_state[:,0,:]
            class_embeddings = model.text_proj(class_embeddings)
            class_embeddings /= class_embeddings.norm(dim=-1, keepdim=True)
...
parser.add_argument('--baseline_alpha', type=float, default=1)
parser.add_argument('--baseline_beta', type=float, default=1.17)
parser.add_argument('--train_epoch', type=int, default=20)
parser.add_argument('--train_epoch_d', type=int, default=80)
parser.add_argument('--augment_epoch', type=int, default=10)
parser.add_argument('--checkpoint', default='../model_large.pth') 
...
def main():
    ...
    train_features_path = "./f_train.pt"
    train_targets_path = "./t_train.pt"

    test_features_path = "./f_test.pt"
    blip_test_features_path = "./blip_f_test.pt"
    test_targets_path = "./t_test.pt"

    image_size = 224 
    blip_model = blip_itm(pretrained=args.checkpoint, image_size=image_size, vit='large')
    blip_model.eval()
    ...
    images = getattr(torchvision.datasets, args.dataset)(data_path, split='val', transform=preprocess)
    loader = torch.utils.data.DataLoader(images, batch_size=64, num_workers=8, shuffle=False)

    classes = imagenet_classes

    train_transform = transforms.Compose([
        transforms.RandomResizedCrop(size=224, scale=(0.5, 1), interpolation=PIL.Image.BICUBIC),
        transforms.RandomHorizontalFlip(p=0.5),
        transforms.ToTensor(),
        transforms.Normalize(mean=(0.48145466, 0.4578275, 0.40821073), std=(0.26862954, 0.26130258, 0.27577711))
    ])
    train_images = getattr(torchvision.datasets, args.dataset)(data_path, split='train', transform=train_transform)

    print(f"{len(classes)} classes, {len(train_images)} training images before sampling, {len(images)} testing images")
    ...
    # ------------------------------------------saving blip testing features-------------------------------------
    print('start saving blip testing image features')
    ...
    if not load_blip:
        ...
        with torch.no_grad():
            for i, (images, target) in enumerate(tqdm(loader)):
                images = images.cuda()
                image_features = blip_model.visual_encoder(images)[:,0,:]
                image_features = blip_model.vision_proj(image_features)
                image_features /= image_features.norm(dim=-1, keepdim=True)
                test_features.append(image_features)

        blip_test_features = torch.cat(test_features)
        torch.save(blip_test_features, blip_test_features_path)

    else:
        blip_test_features = torch.load(blip_test_features_path)
    ...
    zeroshot_weights = zeroshot_classifier(classes, assemble_templates, model) # 1024, 1000
    bert_zeroshot_weights = bert_zeroshot_classifier(classes, empty_templates, blip_model)
    ...
    if show_baseline:
        top1, top5, n = 0., 0., 0.
        logits = 100. * blip_test_features @ bert_zeroshot_weights
        ...
        top1 = (top1 / n) * 100
        top5 = (top5 / n) * 100
        print(f"BLIP Top-1 accuracy: {top1:.2f}, with zero-shot learning")
        print()
    ...
    # ------------------------------------------ Tip-Adapter ----------------------------------------------------
    if show_baseline:
        adapter = Weight_Adapter(model, train_features_path, len(classes), k_shot).cuda()
        adapter.eval()
    ...
    for train_idx in range(args.train_epoch):
        ...
        for i, (images, target) in enumerate(tqdm(train_loader_shuffle)):
            ...
            with torch.no_grad():
                image_features = model.encode_image(images)
                image_features /= image_features.norm(dim=-1, keepdim=True)
                blip_image_features = blip_model.visual_encoder(images)
                blip_image_features = F.normalize(blip_model.vision_proj(blip_image_features[:,0,:]),dim=-1)   

            new_knowledge = adapter.linear1(image_features)
            #
            new_logits = (new_knowledge - 1).exp() @ (train_images_targets)
            logits = alpha * beta * image_features @ zeroshot_weights
            logits = logits + alpha * (1 - beta) * blip_image_features @ bert_zeroshot_weights
            logits = logits + new_logits 
            #
            loss = F.cross_entropy(logits, target)
            loss_value = loss.item()
            correct = accuracy(logits, target)
        ...
        with torch.no_grad():
            ...
            blip_image_features = torch.load(blip_test_features_path).cuda()
            test_labels = torch.load(test_targets_path)
    ...
    # ------------------------------------------ Stochastic Mapping --------------------------------------------------
    print("Stochastic Mapping")
    alpha = args.baseline_alpha
    beta = args.baseline_beta
    adapter = Weight_Adapter(model, train_features_path, len(classes), k_shot).cuda()
    optimizer = torch.optim.AdamW(adapter.parameters(), lr=args.lr, eps=1e-4)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, args.train_epoch_d * len(train_loader_shuffle))
    
    best_top1 = 0
    best_epoch = 0
    for train_idx in range(args.train_epoch_d):
        adapter.train()
        correct_all = 0
        n = 0
        loss_list = []
        print('Train time: {:} / {:}'.format(train_idx, args.train_epoch_d))
        ...
        for i, (images, target) in enumerate(tqdm(train_loader_shuffle)):
            ...
            image_features = F.dropout(image_features, p=args.drop_prob)
            new_knowledge = adapter.linear1(image_features)
            new_logits = ((-1) * (alpha - alpha * new_knowledge)).exp() @ (train_images_targets)
            logits = 100. * image_features @ zeroshot_weights
            logits = logits + new_logits * beta
